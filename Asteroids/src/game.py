...
class Game:
    def __init__(self):
        ...
        self.text_font1 = pygame.font.SysFont(None, 24)
        self.text_font2 = pygame.font.SysFont(None,48)
        ...
        self.pop_ups = []
        self.pop_ups_text_font = pygame.font.SysFont(None, 24)

        self.SMOKE_IMG = pygame.image.load(f"Assets/smoke{(self.level_manager.level-1)%5}.png")
        self.SMOKE_IMG_ENEMY = pygame.image.load(f"Assets/smoke{(self.level_manager.level)%5}.png")
        self.smokes = []
                    ...
                    if asteroid.health <= 0:
                        if asteroid:
                            try:
                                ...
                                self.pop_ups.append(pop_up(10*self.score_multiplier, asteroid.coord,self.pop_ups_text_font))
                                asteroids.remove(asteroid)
                            except:pass
                ...
                for enemy in self.enemies:
                    ...
                    if enemy.mask.overlap(PW.mask, (off_setX, off_setY)):
                        ...
                        if enemy and enemy.health <= 0:
                            self.space.player.score += 100 *self.score_multiplier
                            self.pop_ups.append(pop_up(100*self.score_multiplier, enemy.coord, self.pop_ups_text_font))
    ...
    def update_enemy(self, enemies, surface, dt, target = [0,0]):
        ...
        for enemy in self.enemies:
            ...
            if enemy.smoke_timer > 0.02:
                enemy.smoke_timer = 0
                x = random.randint(-10,10)
                y = random.randint(-10,10)
                self.smokes.append(smoke(enemy.direction,[enemy.coord[0]+x,enemy.coord[1]+y],self.SMOKE_IMG_ENEMY, lifetime=SMOKE_LIFETIME-1))
            for bullet in self.bullets:
    ...
    def show_score(self, screen):
        img = self.text_font1.render(f"Score: {self.space.player.score}",True, "White")
        screen.blit(img, (WIDTH/2-30,70))
        img = self.text_font2.render(f"{self.level_manager.level}",True, "Purple")
        screen.blit(img,(WIDTH/2,30))
    ...
    def show_pop_ups(self,surface,dt):
        for pu in self.pop_ups:
            
            if pu.update(dt):
                self.pop_ups.remove(pu)
            surface.blit(pu.img,pu.coord)
            
    def show_smoke(self,surface,dt):
        for smoke in self.smokes:
            surface.blit(smoke.rotatedSurf,smoke.rotatedRect)
            if smoke.update(dt):
                self.smokes.remove(smoke)
    
    def show_game_over(self,surface,dt):
            go = self.space.Game_Over
            go.update(dt)
            surface.blit(go.img,go.img_rect)